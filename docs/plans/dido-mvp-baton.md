...
- BLU-519: DONE  (wave 5, opus, authoring, has-new-ui: false) — targets SHARED brain, no GBRAIN_HOME. Flip executed 2026-06-29 with explicit user go-ahead AFTER 822-page backup completed.
  FLIP RESULT (all green): schema use → active pack = dido-engagement v1.0.0; sync dry-run → would_apply=0 all 10 types; sync --apply → applied=0 (no-op, existing corpus untouched); review-orphans → 0 orphans; whoknows "ai model selection" → [] (no stakeholder/expertise pages in demo corpus — Expertise Mapper gap-flags); skillpack doctor → ★ 10/10 [endorsed]; stats → 822 pages/14684 chunks UNCHANGED (zero data loss). Backup: ~/gbrain-prod-backup-2026-06-29 (822 files, 43M). Rollback: `gbrain schema use gbrain-base-v2` (+ export --restore-only if ever needed).
